...
export function generateGlsl(transformApplications, shaderParams) {
    ...
    transformApplications.forEach((transformApplication) => {
        ...
        if (transformApplication.transform.type === 'src') {
            fragColor = (uv) => `${shaderString(uv, transformApplication.name, typedArgs, shaderParams)}`;
        }
        else if (transformApplication.transform.type === 'coord') {
            fragColor = (uv) => `${f0(`${shaderString(uv, transformApplication.name, typedArgs, shaderParams)}`)}`;
        }
        else if (transformApplication.transform.type === 'color') {
            fragColor = (uv) => `${shaderString(`${f0(uv)}`, transformApplication.name, typedArgs, shaderParams)}`;
        }
            ...
            f1 =
                typedArgs[0].value && typedArgs[0].value.transforms
                    ...
                    : typedArgs[0].isUniform
                        ? () => typedArgs[0].name
                        : () => typedArgs[0].value;
            fragColor = (uv) => `${shaderString(`${f0(uv)}, ${f1(uv)}`, transformApplication.name, typedArgs.slice(1), shaderParams)}`;
        }
            ...
            f1 =
                typedArgs[0].value && typedArgs[0].value.transforms
                    ...
                    : typedArgs[0].isUniform
                        ? () => typedArgs[0].name
                        : () => typedArgs[0].value;
            fragColor = (uv) => `${f0(`${shaderString(`${uv}, ${f1(uv)}`, transformApplication.name, typedArgs.slice(1), shaderParams)}`)}`;
        }
    });
    return fragColor;
}
