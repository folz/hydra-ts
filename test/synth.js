...
describe.skip('Synth', function () {
    ...
    it('Contains all transforms', function () {
        ...
        expect(events.filter(({ type }) => type === 'add').map(({ method }) => method)).to.have.members(srcNames);
    });
    it('Can be extended', function () {
        ...
        expect(synth.generators).to.be.an('object').and.to.have.all.keys(srcNames);
        expect(events.filter(({ type }) => type === 'add').map(({ method }) => method)).to.include.members(srcNames);
        [
            {
                foo: {
                    type: 'src',
            ...
            events.length = 0;
            synth.init();
            expect(events.filter(({ type }) => type === 'remove').map(({ method }) => method)).to.include.all.members(srcNames);
            expect(events.filter(({ type }) => type === 'add').map(({ method }) => method)).to.have.all.members([...srcNames, 'foo']);
        });
